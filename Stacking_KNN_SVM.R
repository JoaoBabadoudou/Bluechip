...
library(doParallel)
cl <- makeCluster(detectCores() - 1) # Reserve 1 core
...
tune_gridknn <- expand.grid(
  k = c(30)   # Nombre de voisins à tester
)

# Définir une grille de recherche optimale
tune_gridsvm <- expand.grid(
...
knn_model <- train(
  ...
  tuneGrid = tune_gridknn,        # Grille de recherche pour KNN
...
stack_model <- caretStack(
  all.models = list(knn =knn_model, svm = svm_model), 
  method = "gbm", 
  metric = "Accuracy", 
...
predictions <- predict(stack_model , newdata = test_data)

predictions_vector <- ifelse(predictions$X1 > predictions$X0, "1", "0")

predictions_vector <- factor(predictions_vector, levels = c("0", "1"))


confusionMatrix(predictions_vector, test_data$Loan_Status)
...
predictions_test <- predict(stack_model , newdata = data1 )
predictions_vector <- ifelse(predictions_test$X1 > predictions_test$X0, "1", "0")
...
submission <- data.frame(ID = df1$ID, Loan_Status = predictions_vector )
